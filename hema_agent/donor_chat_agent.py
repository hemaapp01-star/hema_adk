# hema_agent/donor_chat_agent.py
...
def create_donor_chat_agent(context: Dict[str, Any]) -> Agent:
    """
    Creates a Donor Chat agent with instructions formatted with actual context values.
    This agent handles conversations with donors about blood donation requests.
...
"""
    ...
    logger.info(f"✅ Created Donor Chat agent with context for {blood_req.get('organisationName')}")
